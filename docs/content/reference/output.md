...
describes what users get. Pick a format with `-o`, or let neurips choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
neurips <command> -o table   # aligned columns for reading
neurips <command> -o jsonl   # one JSON object per line, for piping
neurips <command> -o json    # a single JSON array
neurips <command> -o csv     # spreadsheet friendly
neurips <command> -o tsv     # tab-separated
neurips <command> -o url     # just the URL column
neurips <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
neurips <command> --fields id,title,url
```
...
```bash
neurips <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
neurips <command>            # a table, because this is a terminal
neurips <command> | wc -l    # JSONL, because this is a pipe
```
